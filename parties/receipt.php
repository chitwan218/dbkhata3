...
// If party is pre-selected, fetch receivable amount
if ($party_id && is_numeric($party_id)) {
    ...
    if ($selectedParty) {
        // Calculate full net balance to determine receivable
        $db->query("
            SELECT 
                SUM(CASE 
        ...
        // If party has a receivable (positive balance), suggest that amount
        $autoAmount = $net > 0 ? number_format($net, 2, '.', '') : '';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$party_id || !$amount || $amount <= 0) {
        ...
        $db->query("INSERT INTO transactions (party_id, type, payment_mode, amount, description, date) VALUES (:party_id, 'receipt', 'cash', :amount, :description, :date)");
        ...
        if ($db->execute()) {
            header("Location: profile.php?id=$party_id&message=" . urlencode("Receipt recorded successfully.") . "&type=success");
            exit;
        } else {
...
<form method="post">
    ...
    <div class="mb-3">
        ...
        <?php if ($autoAmount): ?>
            <small class="text-muted">Suggested amount based on receivable.</small>
        <?php endif; ?>
    </div>
